---
# Same helene double-free reproducer as helene-double-free.md, but instead of an
# inline diff this pulls the *actual fix from LKML by link*: thread-compare fetches
# the thread mbox and `git am`s the series onto `commit:`, then runs the reproducer
# twice in parallel -- baseline (no patch) vs patched (fix applied).
#
# Pick a `commit:` that still has the buggy helene.c (any tag before the fix lands);
# adjust if `git am` rejects the patch against your chosen tree.
commit: v6.16
thread-compare: https://lore.kernel.org/lkml/20260701083223.90137-1-pengcan@kylinos.cn/
regex-dmesg: (BUG: KASAN:|invalid opcode|general protection|Oops)
---

# helene double-free reproducer (KASAN) — fix pulled from LKML by link
...
and applies that fix straight from the mailing list via `thread-compare:` (no inline
patch — the diff is fetched from lore and `git am`'d).

`helene_probe()` (the i2c path) allocates `priv` with `devm_kzalloc()` — so **devres
owns it** — but the copied `helene_tuner_ops.release` also `kfree()`s the same
`fe->tuner_priv`. Two owners → one double-free. The module below models that with a
throwaway platform device.

> **Note:** this reproducer models the double-free *pattern* generically; it does not
> bind the real helene tuner (that needs the SoC + I²C hardware). So KASAN fires on
> **both** the baseline and patched runs — the fix touches the real driver, not this
> synthetic module. The value of this example is showing the **link-based patch
> reference** (`thread-compare:` against a lore URL) and the two-run compare harness.
> For an outcome that flips with the patch you'd need the actual device.
