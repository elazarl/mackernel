...
impl Db {
    ...
    pub fn get_job(&self, id: i64) -> Result<Option<Job>> {
        let c = self.lock();
        let mut stmt = c.prepare(
            "SELECT id, created_ms, started_ms, finished_ms, status, phase, exit_code, ram_peak, disk_peak, reaped_ms, summary, repro_summary, result_summary, detail, source_url, title, short_title
             FROM jobs WHERE id=?",
        )?;
        let mut rows = stmt.query(duckdb::params![id])?;
