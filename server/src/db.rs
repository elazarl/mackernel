...
impl Db {
    pub fn open(path: &std::path::Path) -> Result<Self> {
        ...
        conn.execute_batch(
            ...
            -- A terse two-word title generated by the summarizer, shown inline after #id.
    ...
    /// Store/replace a job's terse two-word title (generated at job start).
    pub fn set_short_title(&self, id: i64, title: &str) -> Result<()> {
        self.lock()
            .execute("UPDATE jobs SET short_title=? WHERE id=?", duckdb::params![title, id])?;
        Ok(())
    }
    ...
    pub fn get_job(&self, id: i64) -> Result<Option<Job>> {
        let c = self.lock();
        let mut stmt = c.prepare(
            "SELECT id, created_ms, started_ms, finished_ms, status, phase, exit_code, ram_peak, disk_peak, reaped_ms, summary, source_url, title, short_title
             FROM jobs WHERE id=?",
        )?;
        let mut rows = stmt.query(duckdb::params![id])?;
